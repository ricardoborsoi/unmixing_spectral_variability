function [f,A,M,dM] = bcd_admm(Y,A,M,dM,W,H,gamma,eps_abs,eps_rel,epsilon,varargin)
...
% >  M     initial endmembers matrix (L|P);
...
% > beta                    endm. penalty parameter ('NONE','DISTANCE','MUTUAL DISTANCE','VOLUME')
%
% >> DISTANCE
...
% < A   abundance matrix (P|N);
% < M   endmember matrix (L|P);
% < dM   perturbation matrices (L|NP) (cell 1|N);
% < eps_abs,eps_rel   subproblems stopping criterion;
% < epsilon   global stopping criterion;
%
% Example:
% [f,A,M,dM] = bcd_admm(Y,A,M,dM,W,H,gamma,eps_abs,eps_rel,epsilon,'HYPERPARAMETERS',{muA,muM,mudM},'PENALTY A',alpha,'PENALTY M',{type,beta,Y_proj,Y_bar,U,V},'AL INCREMENT',{tau_incr,tau_decr,mu},'MAX ADMM STEPS',Niter_ADMM);
%-------------------------------------------------------------------------%
%%
