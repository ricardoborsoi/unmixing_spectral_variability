function [x,z,d,mu,res_p,res_d,i] = sunsal_mod2(M,y,varargin)


%% need to change: positivity and add to constant
...
% Local variables
%--------------------------------------------------------------
use_gen_positivity = false;
use_gen_addone     = false;
flag_z_init        = false;
flag_d_init        = false;
flag_mu_init       = false;
...
if (rem(length(varargin),2)==1)
    ...
    for i=1:2:(length(varargin)-1)
        switch upper(varargin{i})
            ...
            case 'GEN_POSITIVITY_CONSTANT'
                use_gen_positivity = 'yes';
                positivity_constant = varargin{i+1};
                if ~sum(lambda(:) ==  0)
                    error('Generalized positivity and sum-to-one constraints only implemented for FCLS!')
                end
                if size(positivity_constant,1) ~= p || size(positivity_constant,2) ~= N
                    error('Inconsistent size of generalized inequality constraint!')
                end
            case 'GEN_ADD_CONSTANT'
                use_gen_addone = 'yes';
                add_to_constant = varargin{i+1};
                if ~sum(lambda(:) ==  0)
                    error('Generalized positivity and sum-to-one constraints only implemented for FCLS!')
                end
                if size(add_to_constant,1) ~= 1 || size(add_to_constant,2) ~= N
                    error('Inconsistent size of generalized equality constraint!')
                end
            case 'z0'
                z_init = varargin{i+1};
                flag_z_init = true;
...
% Verify if both generalized constraints are being used
if ~ (strcmp(use_gen_addone,'yes') && strcmp(use_gen_positivity,'yes'))
    error('For now, the G-FCLS code only works with both (+ and ASC) generalized constraints initialized!');
end
...
if sum(lambda(:) ==  0)  && strcmp(addone,'no')
    ...
    %--------------------------------------------------------------------------
    % Fully constrained  leat squares (FCLS) X >= 0 with generalized
    % constraints
    %--------------------------------------------------------------------------
elseif sum(lambda(:) ==  0)  && strcmp(addone,'yes') && strcmp(use_gen_addone,'yes') && strcmp(use_gen_positivity,'yes')    
    % Redefine x_aux for a more general constant (it was defined with ones(1,N) beforehand)
    x_aux = Aux*add_to_constant;
...
%         z = max(x-d,0);
        ...
        z = - positivity_constant + max(x-d+positivity_constant, 0);
        % minimize with respect to x
